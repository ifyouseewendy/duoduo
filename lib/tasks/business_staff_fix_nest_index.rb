...
class BusinessStaffFixNestIndex < DuoduoCli
  ...
  def start
    ...
    NormalStaff.update_all(nest_index: nil)
    LaborContract.update_all(nest_index: nil)
    clean_logger
    init_logger
    ...
    def process_first_sheet
      ...
      (2..last_row).each do |row_id|
        ...
        begin
          ...
          if ns.present?
            lc = ns.labor_contract
            lc.nest_index = _corporation_id
            lc.save!
          end
        rescue => e
...
BusinessStaffFixNestIndex.start(ARGV)
