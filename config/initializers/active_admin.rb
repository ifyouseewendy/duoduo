ActiveAdmin.setup do |config|
  ...
  config.csv_options = { encoding: 'GBK' }
  ...
  config.namespace false do |default|
    ...
    default.build_menu :utility_navigation do |menu|
      menu.add id: 'current_user', label: -> { current_active_admin_user.name }, url: '#', html_options: { style: :blank } do |sub_menu|
        sub_menu.add label: '用户管理', url: -> { admin_users_path }, priority: 1, if: -> { current_active_admin_user.admin? }
