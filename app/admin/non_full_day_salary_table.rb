ActiveAdmin.register NonFullDaySalaryTable do
  ...
  breadcrumb do
    ...
    if params['q'].present?
      ...
      crumbs << link_to('非全日制工资表', "/non_full_day_salary_tables")
    end
  ...
  index do
    ...
    column :salary_items_display, sortable: :start_date do |obj|
      ul do
        ...
        li( link_to "导入", "/non_full_day_salary_items/import_new?non_full_day_salary_table_id=#{obj.id}", target: '_blank' )
      end
    end
  ...
  filter :start_date, as: :select, collection: -> { NonFullDaySalaryTable.dates_as_filter }
  filter :name
  ...
  form do |f|
    ...
    f.inputs do
      ...
      f.input :status, as: :radio, collection: ->{ NonFullDaySalaryTable.statuses_option }.call
  ...
  # Batch actions
  batch_action :batch_edit, form: ->{ NonFullDaySalaryTable.batch_form_fields } do |ids|
  ...
  member_action :audition_state do
    st = NonFullDaySalaryTable.find(params[:id])
    audition = st.audition
    display = st.audition_display

    admin = current_active_admin_user.admin?
    data = NonFullDaySalaryTable::AUDITION_STAGE.reduce([]) do |ar, k|
      ar << {
        key: k,
