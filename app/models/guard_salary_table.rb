class GuardSalaryTable < ActiveRecord::Base
  ...
  def export_xlsx(view: nil, options: {})
    ...
    if staff_view
      collection = GuardSalaryItem.ransack(options).result
    ...
    Axlsx::Package.new do |p|
      ...
      wb.add_worksheet(name: sheet_name, page_margins: margins, page_setup: setup) do |sheet|
        ...
        stats = columns.reduce([]) do |ar, col|
          if GuardSalaryItem.sum_fields.include?(col)
  ...
  def present_fields(collection:, view: , custom: )
    fields = GuardSalaryItem.columns_based_on(view: view, custom: custom)
